...
class Adminresourcebulkaction extends React.Component {
	constructor(props){
		super(props);
		this.state={
			openPopup:false,
	...
	selectAllcheckbox=(e)=>{
		...
		document.querySelector(".resourcesparentcheck").checked=true;
		 if (ele.checked) {
	...
	bulkDelete=(e)=>{
		...
		let clientbulkid={
			...
			try{
				fetch(Admin.adminResourcesbulkdelete.url,{
						headers: {
		                	"Content-Type" : "application/json",
		                	"X-CSRF-Token" : localStorage.getItem("access-token"),
		                	"Authorization": 'Basic ' + localStorage.getItem("basic-auth"),
		                },
		                method:Admin.adminResourcesbulkdelete.method,
						...
						<div>
							...
							<div className="btn-blok">
								<button onClick={((e)=>{e.preventDefault();this.setState({openPopup:false})
									document.querySelectorAll(".resourceschecked").forEach((item,index)=>{
										document.querySelector(".resourcesparentcheck").checked=false;
										item.checked=false;
									})
...
export default Adminresourcebulkaction;
