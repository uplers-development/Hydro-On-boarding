    		...
    		console.log(data);
				...
				for (var i = 0; i < data.length; i++) {
					 ...
					 if(data[i].field_news_feed_type=="5"){
					 	 ...
					 	 newsfeed.push(<div className="datewise-common-block white-bg-boxshadow"><div className="top-title"><img className='svg' src={site_url+data[i].field_icon} alt="warning-logo"/><h4><span>{data[i].title}</span></h4></div><div className="time-date">Today at {data[i].created}</div>{ReactHtmlParser(data[i].body)}<div className="btn-block">{data[i].field_news_feed_button!=='' ? <button className="btn common-btn-white" type="submit"><span>{data[i].field_news_feed_button}</span></button> : ''}</div></div>);
					 }else if(data[i].field_news_feed_type=="3"){
					 	 newsfeed.push(<div className="datewise-common-block white-bg-boxshadow"><div className="top-title"><img className='svg' src={site_url+data[i].field_icon} alt="issue-logo"/><h4>{data[i].title}</h4></div><div className="time-date">Today at {data[i].created}</div>{ReactHtmlParser(data[i].body)}<div className="btn-block">{data[i].field_news_feed_button!=='' ? <button className="btn common-btn-white" type="submit"><span>{data[i].field_news_feed_button}</span></button> : ''}</div></div>);
					 }
